...
public static class Globals
{
    ...
    static Globals()
    {
        ...
        if (string.IsNullOrEmpty(clientId) || string.IsNullOrEmpty(clientSecret))
            throw new InvalidOperationException("Twitch client ID or secret is not set in environment variables.");
        ...
        if (string.IsNullOrEmpty(botUsername) || string.IsNullOrEmpty(channelName))
        ...
        if (!authFileResponse.HasValue)
            throw new InvalidOperationException("Failed to deserialize the token file.");
        
        AccessToken = authFileResponse.Value.AccessToken ?? throw new InvalidOperationException("Access token is missing in the token file.");
        RefreshToken = authFileResponse.Value.RefreshToken ?? throw new InvalidOperationException("Refresh token is missing in the token file.");
        ExpiresIn = authFileResponse.Value.ExpiresIn.ToString();
        ExpiresAt = DateTime.UtcNow.AddSeconds(authFileResponse.Value.ExpiresIn).ToString("o", CultureInfo.InvariantCulture);
    }
